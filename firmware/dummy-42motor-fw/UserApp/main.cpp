...
//     Change below to fit your situation
    ...
    //always to disable the temperature watch at the beginning
    boardConfig.enableMOtorTemperature = false;
...
/* Event Callbacks -----------------------------------------------------------*/
//we put our code in here to collect temperature data per 1 sec period
//we use a global counter for the quick test
uint8_t  count = 0;
extern "C" void Tim1Callback100Hz()
{
    ...
    if (boardConfig.enableMOtorTemperature)
    {
        count ++;
        if (count == 100)
        {
            boardConfig.motor_temperature = (uint32_t)ADC_Get_Temp();
            //reset counter
            count = 0;
        }
    }
...
void OnButton1Event(Button::Event _event)
{
    switch (_event)
    {
        ...
        case ButtonBase::LONG_PRESS:
            //let's put our temp function in here for test
//            ADC_Get_Temp();
            HAL_NVIC_SystemReset();
            break;
